...
4. Build and hot deploy with `scripts/d2e-ui-hot-deploy.sh concept-sets`. The helper builds with `bunx vite build` and updates both Trex UI resource roots: `/usr/src/data/plugins/@data2evidence/d2e-ui/resources` and `/usr/src/bundled-plugins/d2e-ui/resources`.
