...
3. Build vue-mri with `scripts/d2e-ui-hot-deploy.sh vue-mri` when deploying to a running trex container. Run this outside the sandbox in Codex because it builds with Nx and uses `docker cp`. The helper updates both Trex UI resource roots: `/usr/src/data/plugins/@data2evidence/d2e-ui/resources` and `/usr/src/bundled-plugins/d2e-ui/resources`.
